...
use std::{fs, io::Write, process::{Command, Output}, path::Path, io, env};
...
pub fn create_image() -> String{
    ...
    dockerfile.write("COPY execution.sh / \n".as_bytes()).expect("Failed to write to Dockerfile: COPY");
...
fn create_container(image_id: String) -> Command{
    ...
    let com_path = cur_path.clone() + "/command:/command";
